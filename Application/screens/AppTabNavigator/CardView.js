    ...
    fetch(`http://ec2-52-72-52-75.compute-1.amazonaws.com/get_store_data`, {
      ...
      res => {
        ...
        if (res.length == 0) {
          ...
          this.setState({
            latitude_: Number(res[0].store_gps_longitude),
            longitude_: Number(res[0].store_gps_latitude),
            title: res[0].store_name,
            address: res[0].store_address,
          })
        }
      })
      .catch(err => console.error(err));
    ...
    return (
      <Card style={{ marginLeft: 20, marginRight: 20 }}>
        ...
        <CardItem>
          <Left>
            <Body style={{ marginLeft: 60 }}>
              <Text style={{ fontSize: 15, fontWeight: "bold" }}>{data[0]}</Text>
            </Body>
          </Left>
          <Right>
            <Body>
              <Text style={{ fontSize: 15, fontWeight: "bold" }}>{data[1]}</Text>
            </Body>
          </Right>
        </CardItem>
        <CardItem style={{ flexDirection: 'row' }}>
          ...
          <Icon
            ...
            size={13}
            containerStyle={{ paddingLeft: 230 }}
          />
        </CardItem>
        {/* 가게 다이얼로그 */}
        <Dialog.Container visible={this.state.dialog}>
