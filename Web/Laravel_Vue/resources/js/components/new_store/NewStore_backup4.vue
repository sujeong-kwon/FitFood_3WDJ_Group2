            ...
            <v-form @submit.prevent="register">
                ...
                <v-layout class="mt-4 mb-4" justify-center >
                    <v-flex xs12 sm4 md1>
                        ...
                        <div v-for="(menuImg,i) in menuImgs" v-bind:key="i" >
                            <v-img :src="menuImgs[i][0]"></v-img>
                        </div>
                        <!-- <div id="img_div">
                        </div> -->
            ...
            uploadFile(){
                ...
                for(let i = 0; i<this.select_file.length; i++){ // 선택된 파일 갯수만큼 반복
                    ...
                    // this.menuImgs[name].push(this.select_file[i]);
                        // this.menuImgs[name].push(filename); //수정했음 by 정빈
                    }
                //------------------------정빈---------------------------
                var file_path = upload_file.value;
                if(this.select_file[0]) {
                    this.fr.onload = function () {
                        upload_file.src = this.fr.result;
                    };
                    this.fr.readAsDataURL(this.select_file[0]);
                }
                this.fr.readAsDataURL(this.select_file[0]);
                if (file_path) {
                        ...
                        console.log("파일네임",filename); // 파일이름 확인 ex) lemon.jpg
                    }
                document.getElementById('upload-file').value=[];    //초기화
                document.getElementById('img-name').value='';
                this.attachments = [];
